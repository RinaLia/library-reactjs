...
import { getBook, postBook } from "../redux/action/book";
...
class Home extends Component {
  ...
  async addBook(event) {
    ...
    dataSubmit.set("updated_at", this.state.updated_at);
    dataSubmit.set(
      "id_genre",
      this.state.genreList.filter((i) => i.name === this.state.book_genre)[0].id
    );
    dataSubmit.set("id_status", this.state.book_status);
  ...
  fetchData = async (params) => {
    ...
    this.props.getBook(param).then((response) => {
      console.log("store: ", this.props.book);
      console.log("response: ", response);
      const pageInfo = response.value.data.pageInfo;
      this.setState({
        pageInfo,
        isLoading: false,
        dataBook: response.value.data.data,
      });
      if (param) {
            ...
            <div className="d-flex flex-row w-100 ml-3">
              ...
              <div className="w-100 h-100 d-flex flex-column">
                <div className="top-navbar sticky-top">
                  <TopNavbar
                    className="w-100"
                    search={(query) => this.fetchData(query)}
                  />
                </div>
                <Col>
